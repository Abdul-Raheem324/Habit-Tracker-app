...
import { LineChart, XAxis, YAxis, Tooltip, Legend, Line, BarChart, Bar, ResponsiveContainer, CartesianGrid, PieChart, Pie, Cell } from 'recharts';
import { Sun, Moon, Plus, Edit2, Trash2, RefreshCw, CheckCircle, XCircle, BarChart2, Settings, Home, Calendar, TrendingUp, Sparkles, User, Save, X, Clock, Award, AlignLeft, Target, Repeat, PlusCircle } from 'lucide-react';
...
// const initializeHabits = (habits: Habit[]): Habit[] => {
//   const today = new Date().toISOString().split("T")[0]; // Get today's date in ISO format (YYYY-MM-DD)

//   return habits.map((habit) => {
//     // Check if today's date is already in the history
//     const todayEntryExists = habit.history.some((entry) => entry.date === today);

//     if (!todayEntryExists) {
//       return {
//         ...habit,
//         history: [
//           ...habit.history, // Keep existing history
//           { date: today, value: 0 }, // Add today's entry with value 0
//         ],
//         lastUpdated: new Date().toISOString(), // Update the lastUpdated timestamp
//       };
//     }

//     // If today's date already exists, return the habit as it is
//     return habit;
//   });
// };

// Dynamically initialize habits
...
const COLORS = ['#0088FE', '#00C49F', '#FFBB28', '#FF8042', '#A28BFF', '#FF6B6B', '#4CAF50', '#9C27B0'];
...
const Navbar = ({ onNavigate, activeTab, onToggleTheme, theme }: NavbarProps) => (
  <motion.nav
  ...
  >
    ...
    <div className="space-x-4 flex-1 flex justify-center">
      {["home", "dashboard", "statistics"].map((tab) => (
        <motion.button
          key={tab}
...
const LandingSection: React.FC<LandingSectionProps> = ({ onGetStarted, theme }) => {
...
const features = [
  {
    ...
    description: "Easily monitor your daily habits and stay consistent with your goals.",
    icon: "📊",
  },
  {
  ...
  },
  {
    ...
    description: "Receive reminders and motivational quotes to keep you on track.",
    icon: "💪",
  },
];
...
const FeaturesSection: React.FC<FeaturesSectionProps> = ({theme}) => {
  ...
  className={`py-20 px-6 ${
    theme === "light" ? "bg-gradient-to-b from-blue-50 to-gray-100" : "bg-gray-900"
  }`}
  initial="hidden"
  animate="visible"
...
const QuoteWidget: React.FC<QuoteWidgetProps> = ({ quote, onRefresh, theme }) => {
  return (
    <motion.div
...
//   // Sort history by date (ascending) to ensure proper order
...
//     const currentDay = new Date(sortedHistory[i].date);
//     const previousDay = i > 0 ? new Date(sortedHistory[i - 1].date) : null;

//     // Check if the current day's value meets or exceeds the target
//     if (sortedHistory[i].value >= target) {
//       streak++;
//     } else {
//       break; // Break the streak if the target is not met
//     }

//     // Check for continuity in dates (if there's a gap, break the streak)
//     if (
//       previousDay &&
//       (currentDay.getTime() - previousDay.getTime()) / (1000 * 60 * 60 * 24) > 1
//     ) {
//       break;
//     }
//   }
...
//     (todayValue / habit.target) * 100,
//     100
//   );

//   // Calculate streak dynamically
//   const currentStreak = calculateStreak(habit.history, habit.target);
...
//     const updatedValue = Math.max(0, newValue); // Ensure value is not negative
...
//     // Recalculate streak after updating progress
//     const updatedStreak = calculateStreak(updatedHistory, habit.target);

//     // Call the parent handler with the updated progress and streak
//     onUpdateProgress(habit.id, updatedValue, updatedStreak);
//   };
...
//           }`}
//         >
//           Streak: {habit.streak || currentStreak}  days
//         </span>
//       </motion.div>
...
//             theme === "light" ? "bg-indigo-500" : "dark:bg-indigo-500"
//           }`}
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span
          className={`text-sm font-medium ${todayValue >= habit.target ? "text-green-500" : 
            theme === "light" ? "text-gray-600" : "dark:text-gray-400"
          }`}
        >
      ...
      >
        ...
        <div className="flex space-x-2">
          ...
          >
            <XCircle className={`h-4 w-4 ${todayValue <= 0 ? "opacity-50" : ""}`} />
          </motion.button>
          <motion.button
            onClick={() => handleUpdateProgress(todayValue + 5)}
            className={`p-1 rounded-full transition-colors ${
...
// const Dashboard: React.FC<DashboardProps> = ({ habits, onUpdateProgress, onAddHabit, onEditHabit, onDeleteHabit,theme }) => {
//   return (
//     <div
...
//           theme === "light"
//             ? "text-white bg-indigo-500 hover:bg-indigo-600 focus:ring-indigo-400"
//             : "text-white bg-indigo-700 hover:bg-indigo-600 dark:focus:ring-indigo-500"
...
const Dashboard: React.FC<DashboardProps> = ({
  ...
  useEffect(() => {
    if (filter === "all") {
      ...
      setFilteredHabits(
        habits.filter((habit) => habit.category.toLowerCase() === filter.toLowerCase())
      );
    }
  }, [habits, filter]);

  // Get unique categories for filter
  const categories = ["all", ...new Set(habits.map((habit) => habit.category.toLowerCase()))];
...
// }) => {
//   /**
//    * Helper function to get the day label (e.g., "Mon", "Tue")
//    */
//   const getDayLabel = (date: string) => {
//     const days = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"];
//     const dayIndex = new Date(date).getDay(); // Get day index from date
//     return days[dayIndex];
//   };

//   /**
//    * Helper function to label weeks (e.g., "Week 1", "Week 2")
//    */
//   const getWeekLabel = (weekIndex: number) => `Week ${weekIndex + 1}`;

//   /**
//    * Format chart data for unified rendering
//    * This approach ensures all data points are properly connected
//    */
...
//       // Take last 7 days if more than 7
//       const relevantDates = allDates.length > 7 ? allDates.slice(-7) : allDates;

//       // Create formatted data array with all habits for each date
//       const formattedData = relevantDates.map((date) => {
//         const dataPoint: any = {
//           date,
//           day: getDayLabel(date), // Use day label for the date
//         };
...
//       // Create a map of week labels to record data for each habit
...
//             (entryDate.getTime() - startOfYear.getTime()) /
//               (24 * 60 * 60 * 1000)
//           );
...
//             weeksData[weekLabel] = { week: weekLabel };
//           }

//           // Add or update habit value for this week
//           weeksData[weekLabel][habit.name] =
//             (weeksData[weekLabel][habit.name] || 0) + entry.value;
//         });
//       });
...
//       // Take last 5 weeks if more than 5
//       return formattedData.length > 5
//         ? formattedData.slice(-5)
//         : formattedData;
//     }
...
//               } px-4 py-2 text-sm font-medium rounded-l-md border border-gray-300 border-gray-600`}
//               whileHover={{ scale: 1.05 }}
//               whileTap={{ scale: 0.95 }}
//             >
//               Week
...
//                   ? "bg-white text-gray-700 border-gray-300"
//                   : "bg-gray-700 text-gray-300"
//               } px-4 py-2 text-sm font-medium rounded-r-md border border-gray-300 border-gray-600 border-l-0`}
//               whileHover={{ scale: 1.05 }}
//               whileTap={{ scale: 0.95 }}
//             >
//               Month
  ...
  const prepareChartData = () => {
    if (timeframe === "week") {
      ...
      const formattedData = dates.map(date => {
        const dataPoint: { date: string; day: string; [key: string]: number | string } = {
          date,
      ...
      const weeksData: Record<string, { week: string; weekNumber: number; [key: string]: number | string }> = {};
      const today = new Date();
        ...
        habit.history.forEach(entry => {
          try {
            ...
            if (isNaN(entryDate.getTime())) {
              console.warn(`Invalid date found in habit history: ${entry.date}`);
              return;
            }
            ...
            if (weeksData[weekLabel]) {
              weeksData[weekLabel][habit.name] = (Number(weeksData[weekLabel][habit.name]) || 0) + entry.value;
            }
          } catch (error) {
      ...
      // Convert to array and sort by week number
      const formattedData = Object.values(weeksData).sort((a, b) => a.weekNumber - b.weekNumber);
...
const StatsPage = ({
  ...
  const barChartData = habits.map((habit) => ({
    name: habit.name,
    total: habit.history && habit.history.length > 0
  ...
  const getHabitValueForDay = (habit: Habit, dayStat: WeeklyStat) => {
    ...
    for (const key in dayStat) {
      ...
      if (key.includes(habitName.toLowerCase()) || 
        ...
        {weeklyStats && weeklyStats.length > 0 ? (
          <div className="overflow-x-auto">
            ...
            >
              ...
              <tbody
                className={`transition-colors ${
                  theme === "light" ? "bg-gray-50 divide-gray-200" : "dark:bg-gray-800 dark:divide-gray-700"
                }`}
              >
...
interface FooterProps {
  ...
  theme : string; // Function to update the current view
}
...
const getCurrentDayIndex = () => {
  const dayMapping = [6, 0, 1, 2, 3, 4, 5]; // Maps JS day (0=Sunday) to our array (0=Monday)
  return dayMapping[new Date().getDay()];
};

// const getWeeklyStats = (habits: any[]) => {
//   // Days of the week for the x-axis
//   const days = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"];
  
//   // Initialize weeklyStats with days as the x-axis
//   const weeklyStats = days.map((day) => ({ day }));
  
//   // Populate weeklyStats with each habit's progress
//   habits.forEach((habit) => {
//     habit.history.forEach((progress: any, index: number) => {
//       const habitKey = habit.name.toLowerCase().replace(/\s+/g, "_");
//       (weeklyStats[index] as Record<string, any>)[habitKey] = progress; // Add habit progress to the correct day
//     });
//   });
...
const getWeeklyStats = (habits: Habit[]): WeeklyStat[] => {
  ...
  return Array.from({ length: 7 }, (_, i) => {
    ...
    habits.forEach((habit) => {
      const entry = habit.history.find((h) => h.date === currentDate.toISOString().split("T")[0]);
...
const HabitTrackerApp = () => {
  ...
  const [habits, setHabits] = useState<Habit[]>(loadHabitsFromLocalStorage(initialHabits));

  const [currentEditHabit, setCurrentEditHabit] = useState<Habit | null>(null);
  const [showAddHabitModal, setShowAddHabitModal] = useState(false);
  const [currentTimeframe, setCurrentTimeframe] = useState<"week" | "month">("week");
...
//   // Sort history by date (descending) to start with the most recent
//   const sortedHistory = [...history].sort(
...
//   const todayEntry = history.find(h => h.date === currentDateString);
//   const hasTodayCompletion = todayEntry && todayEntry.value >= target;
  
//   // First, see if today's habit is completed
//   if (hasTodayCompletion) {
//     streak = 1;
//     processedDates.add(currentDateString);
//   }
...
//     // Skip if we've already processed this date (including today)
...
//     // If this is the first iteration and we don't have today's completion
//     if (i === 0 && !hasTodayCompletion) {
//       // Start checking from the most recent entry
//       currentDate = entryDate;
//       currentDateString = entry.date;
//     } else {
//       // For subsequent iterations, check the previous day
//       const expectedPrevDate = new Date(currentDate);
//       expectedPrevDate.setDate(expectedPrevDate.getDate() - 1);
      
//       // If this entry is not for the expected previous day, break the streak
//       if (entryDate.getTime() !== expectedPrevDate.getTime()) {
//         break;
//       }
      
//       // Update the current date we're checking
//       currentDate = entryDate;
//       currentDateString = entry.date;
//     }
    
//     // If the value meets or exceeds the target, increment the streak
//     if (entry.value >= target) {
//       streak++;
//       processedDates.add(entry.date);
//     } else {
//       // If we find an entry that didn't meet the target, break the streak
//       break;
//     }
//   }
...
// };
  ...
  //         ? {
  //             ...habit,
  //             history: habit.history.map((progress, index) =>
  //               index === currentDayIndex ? value : progress
  //             ),
  //             streak: calculateStreak(
  //               habit.history.map((progress, index) =>
  //                 index === currentDayIndex ? value : progress
  //               ),
  //               habit.target
  //             ),
  ...
  return (
    <div className={`min-h-screen font-sans transition-colors ${
      ...
      {activeTab === "home" && (
        <>
          <LandingSection onGetStarted={() => setActiveTab("dashboard")} theme={theme} />
